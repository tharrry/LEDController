...
class prettyLight():
    ...
    def colorWipe(self, strip, color, urgency, wait_ms=50):
    ...
    def light(self, contact, urgency):
        strip.begin()
        ...
        if index == 0:
            ...
            arr = self.textToArr(arr)
            color = Color(0,30,0)
            self.scrollText(arr,color)
        if index == 1:
            for i in "Outlook":
                arr = arr + letters.symbToArr(i)
            arr = textToArr(arr)
